...
def test_read_profile_existing_user(mock_get):
    ...
    mock_claims = {"sub": "test-user-id-123", "username": "testuser"}
    app.dependency_overrides[verify_token] = lambda: mock_claims
...
def test_update_profile(mock_get, mock_save):
    ...
    payload = {"nickname": "New Nickname", "bio": "New Bio"}
    response = client.post(
        "/api/me/profile", json=payload, headers={"Authorization": "Bearer dummy"}
    )
