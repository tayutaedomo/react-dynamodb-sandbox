...
WAF は以下の3箇所にアタッチされており、それぞれの防御層を個別にテストできます。

1. **Amplify Hosting (フロントエンド)**: `test_amplify_waf.sh`
2. **CloudFront (フロントエンド代替)**: `test_cloudfront_waf.sh`
3. **API Gateway (バックエンド)**: `test_api_waf.sh`
...
### 3. CloudFront 側の WAF テスト
フロントエンド（静的サイト配信の代替アーキテクチャ）に対するレート制限をテストします。

```bash
cd scripts/waf-test
./test_cloudfront_waf.sh
```
...
> **⚠️ 注意事項 (Global WAF の反映遅延について)**
> CloudFront や Amplify にアタッチされる Global WAF は、新規作成やルール変更後、世界中のエッジロケーションに反映されるまでに **数分〜最大10分程度の遅延** が発生します。インフラ作成直後にスクリプトを実行して 100回を超えてもブロックされない場合は、バグではなく AWS の仕様による遅延のため、しばらく時間を置いてから再度実行してください。
...
- **CloudFront のブロックログ**: `aws-waf-logs-cloudfront` ロググループ (※ 米国東部 `us-east-1` リージョン)
...
### WAF ブロックログの確認 (CLI)
AWS コンソールを開かずに、コマンドラインから直近10分間の WAF ブロックログ（`BLOCK` アクション）を確認するスクリプトです。引数に `api`, `amplify`, `cloudfront` のいずれかを指定します。
...
**CloudFront のブロックログを確認:**
```bash
./get_waf_logs.sh cloudfront
```
